...
**Per-entry caps.** Trait spends are capped at **200** (`s2flexxpdialog.cpp:127`, `s2flexxpdialog.cpp:190`, `s2flexxpdialog.cpp:361`), skill spends at **35** (`s2flexxpdialog.cpp:214`, `s2flexxpdialog.cpp:362`), and each of the eight attribute spin boxes at **200** (`s2flexxpdialog.cpp:364-371`). When the allowance runs out the dialog freezes every control at its current value rather than clamping to a remainder (`s2flexxpdialog.cpp:349-359`). `S2FXDDisableElem` applies category restrictions, not exclusions for already-granted targets: Military School disables trait and attribute allocations (`s2flexxpdialog.cpp:498-512`), Preparatory School freezes trait spending at a total of 80 (`:514-522`), and Spacer Family freezes skill spending at a total of 100 (`:524-532`). The latter two desktop checks use equality, so an increment can overshoot them.
...
**Stage 1–2 web-port behavior.** The normalized `childhood` catalog in `data/rules/modules.json` drives eligibility, fixed choices, caste overlays, Sibko branches and flex restrictions. Main and advanced choice slots remain independent. Stage 1 More choices are fixed module choices, not this flex allowance. Blue Collar/Farm each own two +5 Interests selections; Born Mercenary Brat retains the single +10 Language selection in §7.4 rather than inheriting a repeat from its filtered dropdown position (`Wizard::S1ChangeElem1` / `S1ChangeElem2`). Source overwrite semantics and literal grant names remain intact.

The web allocator enforces the category limits as inequalities, keeps reductions/removal available at exhaustion, and commits numeric replacements on blur or Enter. Invalid replacements preserve the previous full allocation. Replay applies each module and its owned flex once to the completed prefix; changing an ancestor or confirming Back reconstructs the retained prefix instead of subtracting mutable grants. Draft XP includes the resulting stats, while Wizard XP charges only module costs. Sibko field sets may be empty or partial; unused pool XP does not transfer. Selected fields earn their full grants, with their separate rebate projected only across Stage 2→3 (`Wizard::Stage3Main`), reversed on Back and restored once on re-entry. These port rules live in `lib/characters/{childhood,flex-xp,sibko}.ts` and `app/(app)/characters/new/wizard/wizard-state.ts`; prerequisite reporting and final saving are separate work.
