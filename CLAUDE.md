...
**XP math** (`lib/characters/xp.ts`): 8 attributes start at `ATTRIBUTE_BASE = 100` and only the excess costs XP; budget is 5000 (`CHARACTER_START_XP`); `gmxpmod` is persisted but not read by the port's calculation. **This diverges from the desktop**, which charges the full attribute value (`mainwindow.cpp:830-833`, ~800 XP per character) and derives `gmxpmod` as a load-bearing residual (`mainwindow.cpp:397-401`). See `docs/RULES.md` §2.2 and §2.5; tracked as `docs/PLAN.md` step 10.
